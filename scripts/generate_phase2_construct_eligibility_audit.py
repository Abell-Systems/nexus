#!/usr/bin/env python3
"""Emits the Phase 2 N=39 per-record demand construct eligibility audit (Auditor A pass).

Produced against docs/phase2-demand-construct-eligibility-audit-protocol.md. Reads only
title/description already in the frozen dataset_phase2_demand_corpus_n39.json (no CPC,
no retrieval results, no sector taxonomy) — the same permitted-information scope as
the sector taxonomy amendment (#83)'s D3, applied here to a logically prior question
(construct eligibility, not sector fit).

This is Auditor A's independent pass only. Records marked INELIGIBLE or UNCERTAIN, and
any ELIGIBLE record flagged `needs_auditor_b=true`, require Auditor B review before
#84 can close (see protocol doc, "Roles").
"""
...
# Decision rule (protocol doc): exclusion_criterion_1 -> INELIGIBLE;
# else no technical_problem_present -> INELIGIBLE;
# else no technology_solution_requested -> INELIGIBLE; else ELIGIBLE.
AUDIT_ENTRIES = [
    {
    ...
    },
    {
        ...
        "exclusion_criterion_1": "no",
        "construct_status": "UNCERTAIN",
        "rationale": "Text describes the requesting company's own existing magnet-manufacturing capabilities and solicits collaborators for product commercialization, rather than stating an external technical problem to be solved. Not a clean fit for the exclusion text either (it is not a marketing/consumer-research brief) -- genuinely undecidable from the text alone.",
    ...
    },
    {
        ...
        "exclusion_criterion_1": "no",
        "construct_status": "INELIGIBLE",
        "rationale": "Inverse construct: the requester seeks to acquire existing granted patents for licensing/monetization, not a technology solution to a technical problem. No problem is stated; no external solution is being solicited.",
    ...
    },
    {
        ...
        "exclusion_criterion_1": "yes",
        "construct_status": "INELIGIBLE",
        "rationale": "Broad consumer-trend/design-thinking brief ('understand the needs ... explore how evolving trends can reshape the role') with no defined technical problem or specific technology solicited, despite matching a category name in the taxonomy's protocol trace. Note: docs/data_provenance.md's illustrative Call #2293 description names specific technical requirements (IoT sensors, greywater recycling) not present in this record's actual text -- flagged as a discrepancy for Auditor B, not resolved here.",
    ...
    },
    {
        ...
        "exclusion_criterion_1": "yes",
        "construct_status": "INELIGIBLE",
        "rationale": "Web design/communications deliverable (website redesign), not patent-relevant technology.",
    ...
    },
    {
        ...
        "exclusion_criterion_1": "no",
        "construct_status": "UNCERTAIN",
        "rationale": "Title names a technology domain ('next-generation proteins') but the body never specifies a protein technology, technical spec, or defined problem -- it reads as a regional bioeconomy/food-systems strategy brief ('living lab', 'co-creating, testing and scaling'). Genuinely undecidable between a materials/biotech solicitation and a regional-strategy request from the text alone.",
...
def main() -> int:
    corpus_path = repo_root / "data" / "evaluation" / "dataset_phase2_demand_corpus_n39.json"
    ...
    output_file = output_dir / "phase2_demand_construct_eligibility_n39_v1.json"
    sidecar_file = output_dir / "phase2_demand_construct_eligibility_n39_v1.json.sha256"
