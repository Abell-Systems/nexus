...
def test_invalid_date_rejection():
    ...
    with pytest.raises(ValidationError):
        EvaluationDemand(
            ...
            title="Title",
            description="Desc",
            posted_date="invalid-date-string",  # Must be valid ISO date or date object
            provenance=prov,
        )


def test_provenance_modality_is_sole_truth():
    prov_synth = EvaluationProvenance(
        source_authority="synthetic_generator",
        source_uri="memory://script",
        extraction_timestamp=datetime.now(UTC),
        raw_payload_sha256="b" * 64,
        modality=DataModality.SYNTHETIC_CONTROL,
    )
    demand = EvaluationDemand(
        demand_id="SYNTH-1",
    ...
    assert demand.provenance.modality == DataModality.SYNTHETIC_CONTROL
