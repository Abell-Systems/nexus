...
def test_hierarchical_cpc_similarity_levels():
    from domain.models.matching import compute_cpc_symbol_similarity

    # 1. Exact subgroup match -> 1.00
    assert compute_cpc_symbol_similarity("C11D1/02", "C11D1/02") == 1.00
    assert compute_cpc_symbol_similarity("C11D1/00", "C11D1/00") == 1.00

    # 2. Same main group, different subgroup -> 0.75
    assert compute_cpc_symbol_similarity("C11D1/02", "C11D1/66") == 0.75
    assert compute_cpc_symbol_similarity("C11D1", "C11D1/02") == 0.75

    # 3. Same subclass, different main group -> 0.50
    assert compute_cpc_symbol_similarity("C11D1/02", "C11D3/386") == 0.50
    assert compute_cpc_symbol_similarity("C11D", "C11D3/00") == 0.50

    # 4. Same section, different class/subclass -> 0.25
    assert compute_cpc_symbol_similarity("C11D1/02", "C22C1/00") == 0.25
    assert compute_cpc_symbol_similarity("C11D", "C08L1/00") == 0.25

    # 5. Different section -> 0.00
    assert compute_cpc_symbol_similarity("C11D1/02", "H01M10/0525") == 0.00
    assert compute_cpc_symbol_similarity("C11D", "E03C1/00") == 0.00


def test_compute_max_cpc_similarity_multi_symbols():
    from domain.models.matching import compute_max_cpc_similarity
    ...
    assert compute_max_cpc_similarity(demand_cpcs, patent_cpcs) == 0.50

    # No overlap
    assert compute_max_cpc_similarity(["C11D1/00"], ["H01M4/00"]) == 0.00

    # Empty inputs
    assert compute_max_cpc_similarity([], ["C11D1/00"]) == 0.00
    assert compute_max_cpc_similarity(["C11D1/00"], []) == 0.00
