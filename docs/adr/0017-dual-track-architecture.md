...
* Edits no existing ADR. Known documentary contradictions are recorded in the canonical roadmap (`../roadmap.md`), not repaired here: `docs/architecture.md` still describes the pre-UC1 ip-matchmaker topology, and archived hackathon material still promises ScoreCards "for patent filings". Those reconciliations belong to their respective Lab/Product PRs. (The M0–M6 hypothesis family / study protocol primary-endpoint mismatch this ADR named at the time is resolved as a documentary declaration — see `../roadmap.md` §4 item 3: `nDCG@10` is the confirmatory endpoint; the sealed `comparisons_m0_m6.json` file itself is unchanged and its `MRR` hypotheses are treated as legacy/secondary.)
