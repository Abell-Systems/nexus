...
**Why `k = 1.0`.** The canonical default for a half-saturation constant in this functional family, absent domain-specific calibration data — and per constraint (1), no calibration data is available to use anyway. This is stated as the choice for its lack of free parameters beyond the form's own convention, not because of any effect it has on the 45-pair pilot; note that **the fix's correctness does not depend on which positive `k` is chosen** — `f_lex` is bounded in `[0,1)` for any `k > 0`, so `overall_score`'s boundedness is a property of the transform's *shape*, not its specific parameter. That the specific value is inconsequential to correctness is exactly what makes it safe to fix without looking at benchmark output.
...
- Both transforms are parameter-free or single-parameter with a stated, benchmark-independent justification, so nothing about this decision needs re-deriving when the dataset scales up per the roadmap.
