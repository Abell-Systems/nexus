...
**Leaf semantics: closed publication-date intervals, not calendar labels.** A "month" leaf is not an informal calendar bucket — it is the closed interval `[start_date, end_date]` (both inclusive) where `start_date` is the first day of that month and `end_date` is the *last* day of that month, so a `publication_date` belongs to exactly one leaf with no ambiguity at month boundaries: two contiguous leaves satisfy `next.start_date == prev.end_date + 1 day`. The overall 10-year window composes the same way, with no exception needed: `window_start ≤ publication_date ≤ window_end`, matching ADR 0020 §2's "10 years preceding the freeze cutoff date" (inclusive of the cutoff day itself). The first and last leaves of a jurisdiction's partition tree are therefore clipped to `[window_start, first_month_end]` and `[last_month_start, window_end]` respectively, not full calendar months, whenever the window doesn't align exactly to month boundaries. Partitioning at year granularity uses the same closed convention (`[YYYY-01-01, YYYY-12-31]`), clipped identically at the window's two edges. **This closed-interval representation is authoritative** for `DatePartition`'s `start_date`/`end_date` fields and for the CQL each leaf issues (`pd within "A B"`, both bounds inclusive — matching `ops_query.py`'s existing convention exactly, no translation step): it is not an implementation detail sitting underneath a conceptually half-open contract. An earlier draft of this document described leaf semantics as half-open (`[start, end)`); that was a specification error, corrected here to match what `DatePartition`, `subdivide()`, and `build_partition_cql()` actually implement and what their tests actually verify (contiguity via the `+ 1 day` check above, not an exclusive-end comparison).
...
2. **Disjunction.** Leaf partitions do not overlap by construction (a jurisdiction × year × month cell is claimed by exactly one leaf, per §4's closed-interval semantics — no `publication_date` falls into two leaves). Cross-partition duplicate `publication_id`s are still possible in practice (e.g. a document indexed under more than one date field by OPS) and are resolved exactly as today, downstream, by `select_frozen_patents`'s dedup (PR #57's hard-fail-on-divergent-content policy applies identically here — partitioning changes nothing about that step). Partition boundaries are never used as a deduplication mechanism.
