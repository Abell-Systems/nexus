...
* **Reproducible *A Priori* Power Analysis Procedure:**
  ...
  * **Frozen Demand Sample Size:** $|\mathcal{D}| = 60$ — the minimum $N$ in the pre-registered candidate grid at which $\widehat{\text{Power}}(N) \ge 0.80$ for $\theta = 0.2$ (observed: $\widehat{\text{Power}}(60) = 0.8164$; see `data/experiments/power_analysis_wilcoxon.json` → `results[theta=0.2].power_curve["60"]`). This is frozen **before** any Phase 2 test-split result is inspected, per the Binding Rule above. **This frozen artifact is not modified by the amendment below.**
  * **Amendment (2026-09-07) — Sample size actually available: $|\mathcal{D}| = 48$.** A source feasibility search for real, Spain-origin, `Technology request`-construct demands was conducted against predefined acceptance criteria (public reproducible access, compatible demand construct, origin verified by the real `DefaultOriginResolver`/ADR 0003, deduplicable identifier). Full source-by-source record: `docs/phase2-demand-acquisition-audit.md`. Eleven candidate sources were evaluated; three (InnoGet, the EEN Partnering Opportunities Database, and its Lombardia regional mirror) yielded 48 verified, deduplicated Spain-origin demands; the remaining eight yielded zero usable records (construct mismatch, inactive, or gated behind authentication). This is recorded as an empirical availability ceiling under the stated criteria, not as insufficient search effort. **None of the following were done to reach 60:** the Spain-origin eligibility criterion was not relaxed, no observation was fabricated or imputed, no incompatible demand construct (e.g. funding-consortium partner search) was mixed in, and `jurisdiction_policy.json` was not edited to reclassify ambiguous records as Spanish. A sensitivity power analysis under the identical frozen design (same seed, calibration, $\alpha$, $B$) was computed for $N=48$: $\widehat{\text{Power}}(48) = 0.725$, vs. $\widehat{\text{Power}}(60) = 0.8164$ (see `data/experiments/power_analysis_wilcoxon_n48_sensitivity.json`). **Consequence:** Phase 2 proceeds with $|\mathcal{D}| = 48$; the reduced power to detect the pre-registered small effect ($\theta = 0.2$) is an explicit, declared limitation of any confirmatory result, not a silent deviation.
