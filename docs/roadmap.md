...
| **PR-F** | Phase-2 dataset + DEV/TEST freeze + powered efficacy + WPI | **Partial** — Dev/Test split mechanism done (#79/#93); corpus construct-eligibility audit done (#85/#86), 24-demand eligible subset frozen (#88) toward the `|D|=60` target; organization-level demand independence audit completed (ADR 0029) yielding N=18 independent corpus (`dataset_phase2_independent_corpus_v1.json`) and identifying Dev/Test organization leakage in historical split. **Open:** growing the corpus to `|D|=60`, remaining independence axes (sector, tech-family, problem duplication), family-aware evaluation, uncontaminated Dev/Test split over independent demands, dual annotation/κ at scale, and the powered efficacy run itself | Lab |
