...
**PR-E execution note (2026-09-16):** the table's PR-E scope was executed under issue #104, not under that letter — see the §2 update above. Blinded re-annotation + IAA acceptance criterion: satisfied (κ=0.597 unweighted / 0.916 weighted, gold set frozen). CPC-auto card acceptance criterion ("classifier precision/recall reported") is **inconclusive as specified**: the channel produced zero activations (0/30 demands), so no classifier predictions exist and no precision/recall can be computed. The activation diagnostic itself is closed as a negative result — this is not "the classifier performed poorly," it is "the classifier never fired." PR-E's own exclusion ("must NOT include Phase-2 collection") was not honored literally: the #101–#104 sequence performed real Phase-2-scale corpus acquisition (OEPM, ADR 0035) and dataset freezing (#102/#103) alongside the annotation work, because the annotation had no real pool to run against otherwise — recorded here as a deliberate scope departure, not a silent one. PR-F's DEV/TEST-freeze precondition is consequently already satisfied; PR-F's powered-efficacy and WPI-manuscript acceptance criteria are not.
