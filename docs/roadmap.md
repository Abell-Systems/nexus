...
Net effect: PR-E's acceptance criteria are met for IAA (κ reported) but answered negatively, not affirmatively, for the CPC-auto criterion (no classifier predictions exist to score, since the channel never fires) — recorded as a closed diagnostic, not an open gap. PR-F's dataset-freeze precondition (§3) is also done via #102/#103 and ADR 0035. **Not done:** powered efficacy on the frozen split, and the WPI manuscript — PR-F itself remains open. Undecided next step: whether to annotate a sample of the 553 dense-exclusive pairs to measure dense retrieval's actual relevance quality.
