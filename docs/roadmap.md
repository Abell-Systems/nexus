...
**Update (2026-09-16) — Phase-2 dataset + PR-E scope executed across the internal #101–#104 work sequence, not under those PR letters:** these are commit-message task labels from before this reconciliation, not the GitHub issue/PR of the same numbers — this very reconciliation later took GitHub PR **#104**, unrelated to the internal label; do not conflate the two. TED source admission (ADR 0033/0034), construct/eligibility amendments (#101b–#101d), an independence audit, and a DEV/TEST freeze (#102/#103) closed with `N_power=65 ≥ 60 PASS`. In parallel, a second corpus track (ADR 0035) acquired and qualified an OEPM Spanish-patent corpus (63/60 records, sealed + hash-verified). Across that internal #104 label: dual blinded annotation of a 108-pair at-scale batch (30 Dev demands × BM25-only candidate pools) with IAA computed (108/108 agree within one point, κ=0.597 unweighted / 0.916 quadratic-weighted), 12 disagreements adjudicated, gold set frozen (`ted_at_scale_gold_set_v1.json`). Pool-coverage measured under a pre-registered, explicitly non-"recall" estimand (demand-level coverage 9/30, pool yield 14/108) — initially mislabeled BM25+CPC, corrected same session once found (`docs/phase2-ted-pool-coverage-results.md`). The CPC-auto channel was then diagnosed as structurally inactive (0/30 demands activate any CPC concept — a vocabulary-register gap between procurement prose and patent-title phrasing, not a taxonomy defect), and a CPV→CPC concordance path was closed `SCOPE-FAILED/DEFERRED` (the only official CPV↔NACE correspondence covers Construction only, matching 9/30 demands). A dense-retrieval diagnostic (ADR 0014's pinned multilingual model, isolated venv) then ran: 30/30 demands get a structurally non-empty pool at `min_threshold=0.0`, but only 47/600 dense pairs overlap the existing gold set — 553 pairs, including all 220 across the 11 BM25-zero-pool demands, are dense-exclusive and unscored, with no relevance inferred for any of them.

Net effect: PR-E's IAA acceptance criterion is satisfied (κ reported). The CPC-auto criterion is inconclusive as specified — the channel produced zero activations, so no precision/recall could be computed — which is a closed diagnostic result, not "the classifier performed poorly" and not an open gap. PR-F's dataset-freeze precondition (§3) is also done via #102/#103 and ADR 0035. **Not done:** powered efficacy on the frozen split, and the WPI manuscript — PR-F itself remains open. Undecided next step: whether to annotate a sample of the 553 dense-exclusive pairs to measure dense retrieval's actual relevance quality.
...
**Naming note:** the table's lettered **PR-E** ("Blinded re-annotation + IAA dry-run + CPC-auto card") had **not** been executed as of 2026-09-07. A differently-scoped comparative experiment, the first empirical M0-vs-M1 comparison (#45), was informally called "PR-E" in project discussion around the same time; it is not a substitute for annotation/IAA work and did not close this table row. To avoid compounding the ambiguity, later ad hoc work is referenced here by its actual PR number (e.g. "#48/#49"), not by a reused letter.

**PR-E execution note (2026-09-16):** the table's PR-E scope was executed across the internal #101–#104 work sequence (commit-message task labels, not GitHub issues — this reconciliation itself is GitHub PR #104, a coincidence of numbering, not the same thing), not under the PR-E letter — see the §2 update above. Blinded re-annotation + IAA acceptance criterion: satisfied (κ=0.597 unweighted / 0.916 weighted, gold set frozen). CPC-auto card acceptance criterion ("classifier precision/recall reported") is **inconclusive as specified**: the channel produced zero activations (0/30 demands), so no classifier predictions exist and no precision/recall can be computed. The activation diagnostic itself is closed as a negative result — this is not "the classifier performed poorly," it is "the classifier never fired." PR-E's own exclusion ("must NOT include Phase-2 collection") was not honored literally: the internal #101–#104 work sequence performed real Phase-2-scale corpus acquisition (OEPM, ADR 0035) and dataset freezing (#102/#103) alongside the annotation work, because the annotation had no real pool to run against otherwise — recorded here as a deliberate scope departure, not a silent one. PR-F's DEV/TEST-freeze precondition is consequently already satisfied; PR-F's powered-efficacy and WPI-manuscript acceptance criteria are not.
